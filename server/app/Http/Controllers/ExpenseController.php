...
class ExpenseController extends Controller
{
    ...
    public function update(Request $req, $trip, $expense)
    {
        $data = $req->validate([
            'amount' => 'required|numeric|gt:0',
            'note' => 'nullable|string',
    ...
    public function destroy($trip, $expense)
    {
        try {
            $user = Auth::guard('api')->user();
            $permission = Participant::where('trip_id', $trip)->where('user_id', $user->id)->where('role', 'owner')->exists();
            if (!$permission) {
                return response()->json([
                    'message' => 'You do not have permission to add an activity to this trip'
                ], 403);
