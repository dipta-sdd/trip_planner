...
const submit = async () => {
    try {
        const res = await $fetch(`http://localhost:8000/api/trips/${props.tripId}/destinations`, {
            method: 'POST',
            headers: {
...
const update = async () => {
    try {
        const res = await $fetch(`http://localhost:8000/api/trips/${props.tripId}/destinations/${data.value.id}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${useCookie('token').value}`
...
const destroy = async (id) => {
    try {
        const res = await $fetch(`http://localhost:8000/api/trips/${props.tripId}/destinations/${id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${useCookie('token').value}`
